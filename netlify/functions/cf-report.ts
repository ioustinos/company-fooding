...
export default async (req: Request, _ctx: Context) => {
  ...
  try {
    ...
    // ---- aggregate ----
    const totals = { orders: 0, gross: 0, benefit: 0, topup: 0 }
    const byCompany = new Map<string, { company: string; orders: number; employees: Set<string>; gross: number; benefit: number; topup: number }>()
    const byEmployee = new Map<string, { company: string; name: string; voucher: string; orders: number; gross: number; benefit: number; topup: number }>()
    const byDay = new Map<string, { date: string; orders: number; employees: Set<string>; gross: number; benefit: number; topup: number }>()
    ...
    for (const r of rows) {
      ...
      const c = byCompany.get(cKey) ?? { company: companyName, orders: 0, employees: new Set<string>(), gross: 0, benefit: 0, topup: 0 }
      c.orders += 1; c.gross += r.subtotal; c.benefit += r.benefit_applied; c.topup += r.topup_amount
      if (r.employee_id) c.employees.add(r.employee_id)
      byCompany.set(cKey, c)

      const eKey = `${cKey}::${(voucher).toLowerCase()}`
      const e = byEmployee.get(eKey) ?? { company: companyName, name: empName, voucher, orders: 0, gross: 0, benefit: 0, topup: 0 }
      e.orders += 1; e.gross += r.subtotal; e.benefit += r.benefit_applied; e.topup += r.topup_amount
      byEmployee.set(eKey, e)

      if (r.delivery_date) {
        const d = byDay.get(r.delivery_date) ?? { date: r.delivery_date, orders: 0, employees: new Set<string>(), gross: 0, benefit: 0, topup: 0 }
        d.orders += 1; d.gross += r.subtotal; d.benefit += r.benefit_applied; d.topup += r.topup_amount
    ...
    const perCompany = [...byCompany.values()]
      .map((c) => ({ company: c.company, orders: c.orders, employees: c.employees.size, gross: c.gross, benefit: c.benefit, topup: c.topup }))
    ...
    const perDay = [...byDay.values()]
      .map((d) => ({ date: d.date, orders: d.orders, employees: d.employees.size, gross: d.gross, benefit: d.benefit, topup: d.topup }))
